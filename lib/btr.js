var BTR = (function () {
    ...
    function BTR() {
        ...
        this.lib = {};
        /**
         * Опции BTR. Задаются через setOptions.
         * @type {Object}
         */
        this._options = {};
        this.utils = {
            ...
            applyTemplates: function () {
                var prevCtx = this.ctx,
                    prevNode = this.node;
                var res = this.bt.processBtrJson(this.ctx, this.ctx.block, true);
                if (res !== prevCtx) {
                    this.newCtx = res;
                }
                this.ctx = prevCtx;
    ...
    BTR.prototype = {
        ...
        apply: function (btJson) {
            return this.toHtml(this.setUniq(this.processBtrJson(btJson)));
        },
         ...
         * @returns {Object|Array}
         */
        processBtrJson: function (btJson, blockName, ignoreContent) {
            var resultArr = [btJson],
                nodes = [{json: btJson, arr: resultArr, index: 0, blockName: blockName}],
                node, json, block, blockView, i, l, p, child, subRes,
        ...
        /**
         * проставляет уникальные id каждому элементу внутри блока
         * @param {Object|Array} json
         * @param {String} parentId необязательный
         * @returns {Object|Array}
         */
        setUniq: function (json, parentId, iteration) {
            ...
            if (typeof json === 'object') {
                ...
                if (Array.isArray(json)) {
                    for (var id = 0, l = json.length; id < l; id++) {
                        if (typeof json[id] === 'object') {
                            this.setUniq(json[id], parentId, id);
                        }
                    }
                }
                else {
                    if (json.elem) {
                        if (!json._attrs) json._attrs = {};
                        if (String(parentId) === 'root') {
                            parentId = '';
                        }
                        json._attrs['data-uniq'] = parentId + '.' + iteration.toString(36);
                    } else {
                        if (!json._attrs) json._attrs = {};
                        json._attrs['data-uniq'] = 'root';
                    }

                    if (typeof json._content === 'object') {
                        this.setUniq(json._content, json._attrs['data-uniq']);
                    }
                }
            }
        ...
        toHtml: function (json) {
            ...
            if (typeof json !== 'object') {
                ...
                if (json.block) {
                    var jattr,
                        attrs = json._disableDataAttrGeneration || json.elem || !json.block ? '' : ' data-block="' + json.block + '"', initOptions;
